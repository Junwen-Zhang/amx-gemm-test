...
int Check_result(int *C, int *C1, long M, long N)
{
...
static void packB(uint8_t *B, int k, int n, uint8_t *Bc)
{
    ...
    for (int tn = 0; tn < n; tn += TILE_ROWS)
      for (int i = 0; i < TILE_COLS; i += 4)
        for (int j = 0; j < TILE_ROWS; j++)
          for (int l = 0; l < 4; l++)
          {
            Bc[tk * n + tn * TILE_COLS + i * TILE_ROWS + j * 4 + l] = B[tk * n + tn + i * n + j + l * n];
          }
}
