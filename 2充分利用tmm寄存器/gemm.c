...
void AMX_GEMM_INT8(int32_t *C, int8_t *A, uint8_t *B, long M, long N, long K)
{
  ...
  for (int i = 0; i < M; i += TILE_ROWS)
  {
    for (int j = 0; j < N; j += TILE_ROWS)
    {
      ...
      int k = 0;
      for (; k <= K - (TILE_COLS * 3); k += TILE_COLS * 3)
      {
        _tile_loadd(1, A + i * K + k, K);
        _tile_loadd(2, Bc + k * N + j * TILE_COLS, 64);
        _tile_loadd(3, A + i * K + k + TILE_COLS, K);
        _tile_loadd(4, Bc + (k + TILE_COLS) * N + j * TILE_COLS, 64);
        _tile_loadd(5, A + i * K + k + 2 * TILE_COLS, K);
        _tile_loadd(6, Bc + (k + 2 * TILE_COLS) * N + j * TILE_COLS, 64);
        _tile_dpbsud(0, 1, 2);
        _tile_dpbsud(0, 3, 4);
        _tile_dpbsud(0, 5, 6);
      }
      for (; k <= K - (TILE_COLS * 2); k += TILE_COLS * 2)
      {
        _tile_loadd(1, A + i * K + k, K);
        _tile_loadd(2, Bc + k * N + j * TILE_COLS, 64);
        _tile_loadd(3, A + i * K + k + TILE_COLS, K);
